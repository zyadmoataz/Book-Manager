...
const initialTasks = [{ read: false, genre: "" }];

//Add a filter (e.g. by genre or read/unread) held in state.
function Filter() {
  const { data } = useBooks();
  const filtersByRead = data?.map((book: Book) => book.read);
  const filtersByGenre = data?.map((book: Book) => book.genre);
  const [selectedFilter, setSelectedFilter] = useState(filtersByRead?.[0]);
